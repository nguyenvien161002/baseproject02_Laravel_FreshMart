...
<div class="container-fluid">
    ...
    <div class="table-responesive">
        <form action="{{URL::to('admin/category_product/update')}}" class="form-horizontal" method="POST">
            @csrf
            <input type="hidden" name="id_category" value="{{ $category_product['id'] }}">
            <div class="mb-3 mt-3">
                <label for="name_category">Tên danh mục:</label>
                <input type="text" class="form-control" value="{{ $category_product['name'] }}" name="name_category" placeholder="Nhập tên danh mục">
            </div>
            <div class="mb-3">
